  ...
  class ActiveRecordSearchableIntegrationTest < Test::Unit::TestCase
    ...
    context "ActiveRecord integration" do
      ...
      context "with pagination" do
        ...
        context "and hash searches" do
          should "find first page with five results" do
            search = Tire.search 'active_record_articles', { :query => { :query_string => { :query => 'test*' } }, :sort => { :title => "asc" }, :size => 5, :from => 0 }
          ...
          should "find next page with five results" do
            search = Tire.search 'active_record_articles', { :query => { :query_string => { :query => 'test*' } }, :sort => { :title => "asc" }, :size => 5, :from => 5 }
            results = search.results
            assert_equal 4, results.size
          ...
          should "find not find missing page" do
            search = Tire.search 'active_record_articles', { query: { query_string: { query: 'test*' } }, :sort => { :title => "asc" }, :size => 5, :from => 10 }
            results = search.results
            assert_equal 0, results.size
